...
const registerCandidate = async (req, res) => {
  ...
  try {
    ...
    if (check) {
      res
        .status(200)
        .json({ status: false, check, error: "Already applied for this job" });
    } else {
      const response = await candidate.create(data);
      if (response) {
        const applicant = await job.findOneAndUpdate(
          { _id: response.jobId },
          { $push: { applicants: response._id } },
          { new: true }
        );
...
const updateCandidateStatus = async (req, res) => {
  ...
  try {
    ...
    if (response) {
      ...
      res.status(400).json({ status: false, error: "Failed to update candidate status" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).json({ error: "Internal Server Error" });
